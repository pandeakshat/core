---
id: your-slug               # must match filename (no spaces)
title: "Post Title — Subtitle if needed"
summary: "One-line summary shown on cards."
date: "YYYY-MM-DD"
tags:
  - TagOne
  - TagTwo
cover: "/images/blogs/example.jpg"
readTime: "6 min read"
medium: "https://medium.com/@pandeakshat/example-article"
---

# Test - 2

Start with a short, clear introduction paragraph.

---

## 📘 Overview
Explain what problem or idea the post covers and why it matters.

---

## 🧩 Key Concepts
| Concept | Description |
|----------|-------------|
| Idea 1 | Summary |
| Idea 2 | Summary |

---

## 🚀 Example / Demo
```python
# include code or workflow example here
```

---

## 🔗 References
- [Medium Version]({{medium}})
- [GitHub Project](https://github.com/pandeakshat/your-repo)
